  ...
  return (
    <div className="space-y-6">
      ...
      {query && totalResults === 0 && (
        <div className="text-center py-16 sm:py-20 bg-card rounded-xl border p-6">
          <p className="text-base sm:text-lg text-muted-foreground">No matches found across any module.</p>
        </div>
      )}

      {results.parts.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-lg sm:text-xl font-semibold flex items-center border-b pb-2">
            <Package className="mr-2 h-5 w-5" /> Parts ({results.parts.length})
          </h2>
                ...
                <Card className="hover:border-primary transition-colors cursor-pointer h-full">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-md flex justify-between">
                      ...
                      <span className="text-xs text-muted-foreground font-mono">{part.part_number}</span>
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-xs sm:text-sm text-muted-foreground">{part.vehicle_name} • {part.company_name}</p>
      ...
      {results.customers.length > 0 && (
        <div className="space-y-4 pt-4">
          <h2 className="text-lg sm:text-xl font-semibold flex items-center border-b pb-2">
            <Users className="mr-2 h-5 w-5" /> Customers ({results.customers.length})
          </h2>
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              ...
              <Link href={`/dashboard/customers/${customer.id}`} key={customer.id}>
                <Card className="hover:border-primary transition-colors cursor-pointer h-full">
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs sm:text-sm text-muted-foreground">{customer.location || 'No location'}</p>
                  </CardContent>
                </Card>
              </Link>
            ))}
      ...
      {results.sales.length > 0 && (
        <div className="space-y-4 pt-4">
          <h2 className="text-lg sm:text-xl font-semibold flex items-center border-b pb-2">
            <ShoppingCart className="mr-2 h-5 w-5" /> Sales ({results.sales.length})
          </h2>
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
